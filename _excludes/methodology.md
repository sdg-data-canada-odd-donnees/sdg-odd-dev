...
## Methodology Adopted   
The Global Indicator Framework (GIF) serves as a comprehensive tool for countries to measure and report their progress on the 17 Sustainable Development Goals (SDGs) in a standardized manner. This framework emphasizes global priorities for sustainable development and provides a unified set of indicators that are applicable across different contexts. Each of the 17 SDGs is associated with specific targets, ensuring that progress can be quantitatively assessed.

In contrast, while the Canadian Indicator Framework (CIF) aligns with the SDGs, it is tailored to reflect Canadian priorities and includes 31 ambitions and 76 indicators that are specific to Canada. However, unlike the Global Indicator Framework where every indicator has a target, less than half of the indicators in the CIF currently have a defined target, either quantitative (e.g., target 11.5 *By 2030, 22% of commuters adopt shared or active transportation*) or aspirational (e.g., target 5.2 *Greater representation of women in leadership roles*).  
